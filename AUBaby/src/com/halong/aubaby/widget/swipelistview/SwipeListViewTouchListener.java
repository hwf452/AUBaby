...
package com.halong.aubaby.widget.swipelistview;
...
public class SwipeListViewTouchListener implements View.OnTouchListener {
		...
		if (swipeOpenOnLongPress) {
			frontView.setOnLongClickListener(new View.OnLongClickListener() {
				...
				public boolean onLongClick(View v) {
					Log.v("11111111", String.valueOf(downPosition));
					if (downPosition >= 0) {
	...
	private void openAnimate(View view, int position) {
		setSwipeClosesAllItemsWhenListMoves(true);
	...
	public boolean onTouch(View view, MotionEvent motionEvent) {
		...
		curP.x = motionEvent.getX();
		curP.y = motionEvent.getY();
		...
		case MotionEvent.ACTION_DOWN: {
			...
			downP.x = motionEvent.getX();
			downP.y = motionEvent.getY();
		...
		case MotionEvent.ACTION_UP: {
			...
			if (minFlingVelocity <= velocityX && velocityX <= maxFlingVelocity
				...
				// 滑动超过1/2viewWidth时，frontview移出屏幕
				swap = true;
				swapRight = deltaX > 0;
			}
		...
		case MotionEvent.ACTION_MOVE: {
			// 上下移动的距离和左右移动的距离做对比，判断上下左右移动
			float moveX = downP.x - curP.x;
			float moveY = downP.y - curP.y;
			// 滑动时,向左滑动时，如果是open，子控件获取焦点。向右滑动，子控件获取焦点。上下子控件获取焦点。其他则父容器获取焦点
			if (Math.abs(moveY) > Math.abs(moveX)&&swipeListView.getLastVisiblePosition() < (swipeListView
					 .getAdapter().getCount() - 1)&&swipeListView.getFirstVisiblePosition() > 0) {
				closeOpenedItems();
				// 上下滑动
				view.getParent().requestDisallowInterceptTouchEvent(false);

			} else if (downPosition != ListView.INVALID_POSITION
					&& opened.get(downPosition) && moveX < 0) {
				// 在item上向左滑动
				// 此句代码是为了通知他的父容器现在进行的是本控件的操作，不要对我的操作进行干扰
				view.getParent().requestDisallowInterceptTouchEvent(true);
			} else if (downPosition != ListView.INVALID_POSITION && moveX > 0) {
				// 在item上向右滑动
				view.getParent().requestDisallowInterceptTouchEvent(true);
			} else {
				closeOpenedItems();
